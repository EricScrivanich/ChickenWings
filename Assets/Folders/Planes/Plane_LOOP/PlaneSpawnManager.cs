...
public class PlaneSpawnManager : MonoBehaviour
{
    ...
    [SerializeField] private float minJetIterationDelay = 2;
    [SerializeField] private float maxJetIterationDelay = 4;

    [SerializeField] private float minCargoOffset = 3;
    [SerializeField] private float maxCargoOffset = 5;
    [SerializeField] private float minCargoIterationDelay = 5;
    [SerializeField] private float maxCargoIterationDelay = 7;
    ...
    IEnumerator SpawnCropPlanes()
    {
        while (true) // Main game loop
        {
            for (int i = 0; i < cropPlanesAvailable; i++)
            {
                ...
                yield return new WaitForSeconds(cropSpawnOffsetVar);
                GetCrop(); // Spawn a plane
                // Wait for offset
            }
            float cropIterationEndDelay = Random.Range(minCropIterationDelay, maxCropIterationDelay);
            yield return new WaitForSeconds(cropIterationEndDelay); // End-of-iteration delay
    ...
    IEnumerator SpawnJetPlanes()
    {
        while (true) // Main game loop
        {
            for (int i = 0; i < jetPlanesAvailable; i++)
            {
                ...
                yield return new WaitForSeconds(jetSpawnOffsetVar);
                GetJet(); // Spawn a plane
                // Wait for offset
            }
            float jetIterationEndDelay = Random.Range(minJetIterationDelay, maxJetIterationDelay);
            yield return new WaitForSeconds(jetIterationEndDelay); // End-of-iteration delay
                                                                   // Optional: Adjust cropPlanesAvailable and delays based on game conditions
        }
    }
    IEnumerator SpawnCargoPlanes()
    {
        while (true) // Main game loop
        {
            for (int i = 0; i < cargoPlanesAvailable; i++)
            {
                ...
                yield return new WaitForSeconds(cargoSpawnOffsetVar);
                GetCargo(); // Spawn a plane
                // Wait for offset
            }
            float cargoIterationEndDelay = Random.Range(minCargoIterationDelay, maxCargoIterationDelay);
            yield return new WaitForSeconds(cargoIterationEndDelay); // End-of-iteration delay
                                                                     // Optional: Adjust cropPlanesAvailable and delays based on game conditions
        }
    }
